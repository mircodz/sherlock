...
public enum SectionType : uint
{
    ...
    // Address and own-size are NOT stored (they're the graph's columns re-permuted): NodeByRpo maps
    // each RPO slot to its object id, and on load address/own are looked up from the graph.
