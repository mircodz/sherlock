...
public sealed class HeapGraphExtractor(DumpSession session)
{
    ...
    public HeapGraph Extract(CancellationToken cancellationToken = default)
    {
        ...
        ClrSegment[] segments = heap.Segments.Where(s => s.ObjectRange.Length > 0).OrderBy(s => s.ObjectRange.Start).ToArray();
        ...
        int workers = session.DataTarget.DataReader.IsThreadSafe ? Math.Clamp(Environment.ProcessorCount, 1, MaxWorkers) : 1;
