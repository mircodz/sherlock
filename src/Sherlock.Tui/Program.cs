...
// PoC: a Sherlock heap explorer on Tessera. A Navigator drill-down (Snapshots -> Snapshot workspace
// -> Instances -> Instance detail); the workspace is a Tabs "lens" bar: Health · Types · Retention ·
// Allocations. See docs/design/tui.md.
//
...
Widget Workspace(Snapshot snap, string id) =>
    new Tabs()
        .Add("Health", HealthTab(snap, id))
        .Add("Types", TypesTab(snap))
        .Add("Retention", RetentionTab(snap))
        .Add("Allocations", AllocationsTab(snap));
...
// Retention: the dominator tree. Roots ARE the top dominator objects; children come lazily from the
// dominator tree. The type and the address are both links (type → instances, address → inspect).
...
Widget AllocationsTab(Snapshot snap)
{
    ...
    var flame = new FlameGraph<AllocationTreeNode>
    {
        LabelSelector = n => n.Label,
        Root = ToFlame(rootNode, "all allocations"),
    };
    ...
    tabs.Add("Call tree", FlamePanel(flame, rootNode.AllocBytes, " Allocation flow — width = bytes ",
...
// By type: what was allocated, largest churn first, with the survived share and how concentrated the
// origin is (few sites = easy to reason about). At the top level, Enter → that type's call tree
// (where it came from); this is the first "where" hop.
...
// Where a given type was allocated: the call tree of just that type's sites (an aggregate of many
// stacks → a flamegraph is right).
Widget TypeAllocPage(Snapshot snap, string typeName)
{
    ...
    // A type's origins are a narrow chain, not a wide overview — a tree-table with per-frame numbers
    // reads better than a flamegraph here (see the flame-vs-tree note in docs/design/tui.md).
...
// the given top frames (the actual nodes, not a synthetic title), children lazy + expanded. Each
// frame links to its callers. Used for focused drills where numbers matter more than shape.
...
// A flamegraph with a live info bar above it: zoom breadcrumb + the selected frame's bytes, share of
// total, and object count. Updated on select/zoom (mirrors the Tessera demo).
...
Widget InstancePage(Snapshot snap, ulong address, string tab = "Inspect")
{
    ...
    var tabs = new Tabs()
        .Add("Inspect", new Panel(inspect, " Object — click a reference to follow it ") { BorderStyle = BorderStyle.Rounded })
        .Add("GC roots", new Panel(roots, " Why it's alive — click a holder to inspect it ") { BorderStyle = BorderStyle.Rounded });
    ...
    if (snap.HasCorrelation && snap.WhoAllocated(address) is { } stack)
    {
        ...
        tabs.Add("whoalloc", Hinted(new Panel(alloc, " Allocation stack — Enter a frame for its callers ") { BorderStyle = BorderStyle.Rounded },
...
// Clicking a method lands here: what it allocates (by type — the concrete answer), then who calls it
// (back-traces). "Method → flamegraph" was too abstract a landing; lead with the type breakdown the
// type dimension now makes possible.
Widget MethodPage(Snapshot snap, string method)
{
    ...
    // A back-trace is a focused drill → tree-table with numbers, not a flamegraph.
    AllocationTreeNode callers = AllocationTreeNode.BuildCallers(profile, method);
    tabs.Add("Callers", callers.Children.Count > 0
...
sealed record ObjTarget(ulong Address, string Tab = "Inspect");
sealed record TypeTarget(string Type);           // → that type's live instances
sealed record TypeAllocTarget(string Type);      // → where that type was allocated (call tree)
sealed record MethodTarget(string Method);
sealed record RootRow(string Text, ulong? Address);
